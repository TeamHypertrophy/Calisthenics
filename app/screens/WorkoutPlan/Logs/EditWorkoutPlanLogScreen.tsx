...
export const EditWorkoutPlanLogScreen: FC<EditWorkoutPlanLogScreenProps> = observer(
  function EditWorkoutPlanLogScreen(_props) {
    ...
    const { logID, planName, initialDate } = _props.route.params
    ...
    const editLog = useMutation({
      mutationFn: async (data: { date: string }) => {
        const response = await api.editWorkoutPlanLog(logID, data)
        if (!response.ok || !response.data) {
          throw new Error("Failed Updating Workout Plan Log")
        }
        return response.data
      },
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["workoutPlanLogs", userID] })
        queryClient.invalidateQueries({ queryKey: ["workoutPlanLog", logID] })
        renderToast("Success", "Workout Plan Log Updated Successfully", "success")
        navigation.goBack()
      },
      onError: (error) => {
        console.error("Error Updating Workout Plan Log:", error)
        renderToast("Error", "Failed to update workout plan log", "error")
      },
    })

    const deleteLog = useMutation({
      mutationFn: async () => {
        const response = await api.deleteWorkoutPlanLog(logID)

        if (!response.ok) {
          throw new Error("Failed to delete workout plan log")
        }
        return response.data
      },
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["workoutPlanLogs", userID] })
        renderToast("Success", "Workout plan log deleted successfully", "success")
        deleteModalRef.current?.close()
        navigation.goBack()
      },
      onError: (error) => {
        console.error("Error deleting workout plan log:", error)
        renderToast("Error", "Failed to delete workout plan log", "error")
        deleteModalRef.current?.close()
      },
    })
      ...
      <>
        ...
        >
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            <Text preset="heading" text={`Edit Log: ${planName}`} style={themed($title)} />

            <View style={themed($dateContainer)}>
              <Text preset="formLabel" text="Workout Plan Date" />
        ...
        <Modalize ref={deleteModalRef} adjustToContentHeight>
          <View style={themed($modalContainer)}>
            <Text preset="subheading" text="Delete Workout Plan Log" style={themed($modalTitle)} />
            <Text style={themed($modalText)}>
              Are you sure you want to delete this workout plan log? This action cannot be undone.
            </Text>
